      ...
      class IndexDBStorage {
        ...
        static async init() {
          ...
          return new Promise((resolve, reject) => {
            ...
            req.onupgradeneeded = e => {
              ...
              // 修正前はdb.objectStoreNames.contains(this.name)（データベース名
              // 'zenza_hls'）を見ていたが、実際に作成しているストア名は'ts-data'
              // （this.storeNames[0]）であり、データベース名と一致することはない。
              // そのためverを上げてもこの分岐が発火せず、次のcreateObjectStoreで
              // 既存ストアと衝突してConstraintErrorになりうる不具合があった。
        ...
        getCount(store, key, {index, timeout}) {
          return new Promise((resolve, reject) => {
            ...
            // 修正前は`err => resolve`で、resolve関数自体を返すだけでresolveを
            // 呼び出しておらず、IndexedDBのエラー時にPromiseが確定しないまま
            // 残っていた（timeout指定時はフォールバックのsetTimeoutで最終的に
            // rejectされるが、無駄に待たされていた。timeout未指定の呼び出し方が
            // あった場合は永久にpendingのままになる）。
            req.onerror = reject;
            if (timeout) {
              setTimeout(() => reject(`timeout: key${key}`), timeout);
            }
    ...
    const ZenzaVideoElement = (({Hls, throttle}) => {
      ...
      const createFragmentLoader = (Hls) => {
        ...
        const frag2hash = fragment => {
          ...
          // 修正前は`this.levels`だったが、frag2hashはアロー関数で、
          // レキシカルなthis（このスクリプトの実行コンテキストではトップ
          // レベルのグローバルオブジェクト）に固定されており、呼び出し方
          // （FragmentLoaderClass.frag2hash(...)等）を変えてもthisは
          // 決して書き換わらない。一方、_onHLSJSManifestParsedが実際に
          // levelsを書き込んでいるのはFragmentLoaderClass（下でこの外側の
          // 変数に代入される）自身なので、そちらを直接参照する必要がある。
          const levels = FragmentLoaderClass && FragmentLoaderClass.levels;
...
大きめの値を入れると最初から高画質が選択されやすくなる。
  ...
  try {
    // 修正前は`if (Hls)`（宣言されていない裸の識別子）だった。@requireの
    // hls.jsがブロックされる等でwindow.Hlsが一度も設定されないと、ここで
    // ReferenceErrorが即座に投げられ、直後のcatchで握りつぶされてしまい、
    // 本来ここで案内するはずだった「読み込み失敗」のフォールバックUI
    // （下のelse if節）が一度も実行されなくなっていた。
    if (window.Hls) {
      console.log('hls.js@%s required', window.Hls.version);
    } else
