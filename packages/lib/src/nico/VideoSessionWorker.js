...
// createStoryboardClasses(util) is the single source of truth for the
// storyboard session/loader classes. It is used two ways:
//   1. Here in the module's normal scope, so tests (and any other
//      non-Worker consumer) can `import {createStoryboardClasses}` and
//      exercise these classes directly.
//   2. Injected as source text into the cross-message Worker created
//      below (see `inject` in the call to
//      workerUtil.createCrossMessageWorker) so the *same* implementation
//      also runs inside the actual Worker thread, where `func`'s own
//      `util` (Worker-aware fetch wrapper) is passed in.
// Do not define these classes a second time inside `func` — see Task 018
// (docs/design-pack/24_TASK_018_STORYBOARD_TEST_BLOCKED.md) for why they
// used to be un-exportable and how this factory fixes that.
const createStoryboardClasses = (util) => {
  class StoryboardInfoLoader {
    ...
    async getStoryboardWithImages() {
      const storyboard = this.getStoryboardWithUrls();
      return {
        ...storyboard,
        images: await StoryboardInfoLoader.fetchImages(storyboard.images),
      };
    }

    // Task 070: 画像（1枚 約0.5MB・10x10コマ）は全ページ同時に取得する。認証は不要（credentials omit で取得可）。
    static async fetchImages(images) {
    ...
    get storyboard() {
      ...
      return {
        ...
        images: images.map(image => {
          // Task 070: 元データを書き換えない（getterが複数回呼ばれるとURLが二重に置換され壊れていた）
          if (/^https?:/.test(image.url)) {
            return {...image};
          }
          const url = new URL(this._url);
          url.pathname = url.pathname.replace(/storyboard\.json$/, image.url);
...
const VideoSessionWorker = (() => {
  const func = function(self) {
    ...
    const storyboard = async ({videoInfo, serverType, withImages = true}) => {
      ...
      await loader.load();
      try {
        const sbInfo = withImages ?
          await loader.getInfo() :
          {...loader.toJSON(), storyboard: loader.getStoryboardWithUrls()};
        return {
          ...sbInfo,
          status: 'ok',
    ...
    // Task 070: URLだけの情報を先に返して表示を始め、画像本体は後からまとめて取得してキャッシュ用に返す
    const storyboardImages = async ({sbInfo}) => {
      const images = await StoryboardInfoLoader.fetchImages(sbInfo.storyboard.images);
      if (images.some(image => !image.buffer)) {
        return {watchId: sbInfo.watchId, status: 'fail'};
      }
      return {
        ...sbInfo,
        storyboard: {...sbInfo.storyboard, images},
      };
    };
    ...
    self.onmessage = async ({command, params}) => {
      switch (command) {
        ...
        case 'storyboardImages':
          return await storyboardImages(params);
      }
    };
  };
  ...
  const initWorker = () => {
    ...
    worker = worker || workerUtil.createCrossMessageWorker(func, {
      name: 'VideoSessionWorker',
      // Give the Worker thread the same storyboard classes `func` uses,
      // built from the single `createStoryboardClasses` factory defined
      // at the top of this module (see the comment there). `inject` is
      // spliced as source text into the generated Worker script, in the
      // same lexical scope that `func`'s own body runs in (see
      // packages/lib/src/infra/workerUtil.js), so `func` can reference
      // `createStoryboardClasses` as if it were declared right above it.
      inject: `const createStoryboardClasses = ${createStoryboardClasses.toString()};`,
    });
  };
  ...
  // Task 070: 高速化
  //  1. キャッシュ(IndexedDB)の確認とセッション作成を同時に始める（キャッシュが無い時に待たない）
  //  2. セッション作成後、画像を待たずにURLだけの情報で表示を始める（シーク時に必要なページだけ描画側が取得）
  //  3. 画像本体は裏で一括取得し、揃ったらキャッシュに保存する（次回から通信なしで即表示）
  const storyboard = async ({type, info}) => {
    const videoInfo = info.toJSON();
    const cacheId = `${videoInfo.watchId}_${type}`;
    const cachePromise = withTimeout(
      StoryboardCacheDb.get(cacheId).catch(() => null), 2000);
    ...
    await workerPromise;
    const params = {videoInfo, serverType: type, withImages: false};
    const result = await worker.post({command: 'storyboard', params});
    if (result && result.status === 'ok') {
      worker.post({command: 'storyboardImages', params: {sbInfo: result}})
        .then(full => full && full.status === 'ok' && StoryboardCacheDb.put(cacheId, full))
        .catch(err => console.warn('storyboard image cache fail', err));
    }
    return result;
  };
...
export {VideoSessionWorker, createStoryboardClasses};
