...
class StoryboardView extends Emitter {
  ...
  _updateFail() {
    // Task 067: ClassList().remove() は自身を返さない（戻り値undefined）ため、
    // 以前の remove(...).add(...) の連鎖は、ストーリーボードが無い動画を開くたびに
    // "Cannot read properties of undefined (reading 'add')" で例外になっていた。
    // あわせてクラス名の綴り間違い（is-uccess）も修正し、ビューが未生成の時は何もしない。
    if (!this._view) {
      return;
    }
